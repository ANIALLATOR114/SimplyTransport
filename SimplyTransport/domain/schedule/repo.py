...
class ScheduleRepository():
    ...
    async def get_schedule_on_stop_for_day(self, stop_id: str, day: DayOfWeek):
        ...
        statement = select(StopTimeModel).options(joinedload(StopTimeModel.trip)).where(StopTimeModel.stop_id == stop_id, StopTimeModel.trip_id == TripModel.id, TripModel.service_id == CalendarModel.id, TripModel.route_id == RouteModel.id, *conditions).order_by(StopTimeModel.arrival_time)
