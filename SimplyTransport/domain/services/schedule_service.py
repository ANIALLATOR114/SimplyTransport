...
class ScheduleService:
    def __init__(self, schedule_repository: ScheduleRepository, calendar_date_respository: CalendarDateRepository):
    ...
    async def get_schedule_on_stop_for_day(self, stop_id: str, day: DayOfWeek) -> list[StaticSchedule]:
        ...
        schedules_from_db = await self.schedule_repository.get_schedule_on_stop_for_day(stop_id=stop_id, day=day)
        static_schedules = [
            StaticSchedule(route=schedule.RouteModel, stop_time=schedule.StopTimeModel, calendar=schedule.CalendarModel)
            for schedule in schedules_from_db
        ]

        return static_schedules

    async def remove_exceptions_and_inactive_calendars(self, static_schedules: list[StaticSchedule]) -> list[StaticSchedule]:
        """Removes exceptions from the list of schedules"""
        current_day = date.today() + timedelta(days=1)
        exceptions_from_db = await self.calendar_date_respository.get_removed_exceptions_on_date(date=current_day)
