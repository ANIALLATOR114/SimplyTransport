...
class StatsController(Controller):
    ...
    async def static_stats(
        ...
        stats_with_percentages = (
            stats_service.convert_stats_to_stats_with_percentage_totals(stats)
        )
    ...
    async def operator_stats(
        ...
        routes_with_percentages = (
            stats_service.convert_stats_to_stats_with_percentage_totals(routes, total_row_key="Total Routes")
        )

        trips = await stats_repo.get_statistics_most_recent_by_type(StatisticType.OPERATOR_TRIP_COUNTS)
        trips_with_percentages = (
            stats_service.convert_stats_to_stats_with_percentage_totals(trips, total_row_key="Total Trips")
        )

        return Template(
            template_name="stats/operator_data.html",
            context={"routes_with_percentages": routes_with_percentages,
    ...
    async def stop_features(
        ...
        stats = await stats_repo.get_statistics_most_recent_by_type(
            StatisticType.STOP_FEATURE_COUNTS
        )
        stats_with_percentages = (
            stats_service.convert_stats_to_stats_with_percentage_totals(stats, total_row_key="Total Stops")
        )

        return Template(
            template_name="stats/stop_features_data.html",
            context={
                "stats_with_percentages": stats_with_percentages
            },
        )
