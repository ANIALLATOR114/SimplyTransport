...
class MapsController(Controller):
    ...
    async def maps(self, agency_repo: AgencyRepository, agency_id:str) -> Template:
        ...
        try:
            ...
            logger.bind(agency_id=agency_id, path=f"{MAPS_TEMPLATES_DIR}/{agency_id}.html").error(f"Route map for agency {agency_id} not found")
            return Template("maps/agency_route.html", context={"agency": agency, "error": "Sorry, this map is not available"})
    ...
    async def static_agency_route_map(
        self, agency_id: str
    ) -> Template:
