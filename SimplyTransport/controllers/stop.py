...
class StopController(Controller):
    ...
    @get("/search", summary="Search stops by name or code", description="Search is case insensitive", raises=[NotFoundException])
    async def search_stops_by_name_or_code(self, repo: StopRepository, limit_offset: LimitOffset, search: str = Parameter(query="search", required=True, description="Search string to search by name or code")) -> OffsetPagination[Stop]:
        try:
