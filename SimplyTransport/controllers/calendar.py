...
class CalendarController(Controller):
    ...
    @get("/date/{date:date}", summary="Get all active calendars on a given date", description="Date format = YYYY-MM-DD")
    async def get_active_calendars_on_date(
        self, repo: CalendarRepository, date: date
    ) -> Calendar:
        result = await repo.list(OnBeforeAfter(field_name="start_date", on_or_before=date, on_or_after=None),
