...
class RealtimeController(Controller):
    ...
    async def realtime_stop(self, stop_id:str, stop_repo: StopRepository, route_repo: RouteRepository) -> Template:
        stop = await stop_repo.get(stop_id)
        routes = await route_repo.get_by_stop_id(stop.id)
        current_time = datetime.now()
        return Template(template_name="realtime/stop.html", context={"stop": stop, "current_time": current_time, "routes": routes})
